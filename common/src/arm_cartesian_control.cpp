    ...
    KDL::JntArray& jntVel)
{
    ...
    for (int i = 0; i < jntVel.data.rows(); i++)
    {
        ...
        float angular_margin = upper_limit - lower_limit;
        if (vel < 0.0)
        {
            angular_margin = pos - lower_limit;
        }
        else
        {
            angular_margin = upper_limit - pos;            
        }
        
        weight_js (i,i) = angular_margin/(upper_limit - lower_limit);
...
void Arm_Cartesian_Control::process(
    ...
    KDL::Twist& targetVelocity, KDL::JntArrayVel& out_jnt_velocities, Eigen::VectorXd& sigma)
{
    ...
    if (jntVel.data.norm() > 0.05)
    {
        ...
        if (max_velocity > max_joint_vel)
        {
            ...
            std::cout << "Max joint velocity reached" << std::endl;
        }
    }
    else
    {
